...
def assignWork(request):
    ...
    if form.is_valid():
        ...
        if currentTaskerId == currentUserId:
            ...
            #work_instance.assignerId = request.user.username 
            #ork_instance.save()
...
def workdetails(request,wid):
    ...
    if workdetails.taskerId != request.user.username and workdetails.assignerId != request.user.username and not request.user.is_superuser:
        from django.core.exceptions import PermissionDenied
        raise PermissionDenied("403 Forbidden: You do not have permission to view this private work record.")
...
def viewRequest(request):
    requests = Requests.objects.filter(destinationEmployeeId=request.user.username).order_by('-id')[:5]
...
def deletework(request, wid):
    ...
    logger.warning(f"AUDIT ALARM: User '{request.user.username}' successfully deleted Task ID: {wid}")
    obj.delete()
